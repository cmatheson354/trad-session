// ToolsDrawer — tuner, recordings, import/export, and list management
export default function ToolsDrawer({
  ...
  const Toggle = ({ icon, label, active, onToggle }) => (
    <button
      onClick={onToggle}
      className={`w-full flex items-center justify-between gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${
        active ? 'bg-yellow-600/30 border border-yellow-600 text-yellow-200' : 'bg-gray-800 hover:bg-gray-700 text-gray-200'
      }`}>
      <div className="flex items-center gap-3">
        <span className="text-lg w-6 text-center">{icon}</span>
        <span>{label}</span>
      </div>
      <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${active ? 'bg-yellow-600 text-white' : 'bg-gray-700 text-gray-400'}`}>
        {active ? 'ON' : 'OFF'}
      </span>
    </button>
  )
